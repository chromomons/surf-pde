...
from stokes_solvers import moving_ns, moving_ns_direct
...
problem_name = "evolving_surface_navier_stokes"
...
# flag that indicates if the solution is solenoidal or not,
# this dictates whether iterative or direct solver will be used
is_solenoidal = soln_params['is_solenoidal']
...
for nref in range(max_nref+1):
    ...
    if is_solenoidal:
        ndof, ts, l2uss, h1uss, l2pss, h1pss = moving_ns(
            mesh=mesh, exact=exact, dt=dt, tfinal=tfinal, order=space_order, time_order=time_order, band=miniband,
            linear_solver_params=linear_solver_params, vtk_out=vtk_out_postfix, logs=solver_logs,
            printrates=print_sparse_solver_rates
        )
    else:
        ndof, ts, l2uss, h1uss, l2pss, h1pss = moving_ns_direct(
            mesh=mesh, exact=exact, dt=dt, tfinal=tfinal, order=space_order, time_order=time_order, band=miniband,
            vtk_out=vtk_out_postfix, logs=solver_logs
        )
